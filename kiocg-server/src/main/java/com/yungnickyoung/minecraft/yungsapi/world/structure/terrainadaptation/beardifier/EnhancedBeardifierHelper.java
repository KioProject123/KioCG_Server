...
public class EnhancedBeardifierHelper {
    ...
    public static Beardifier forStructuresInChunk(StructureManager structureManager, ChunkPos chunkPos, Beardifier original) {
        ...
        Beardifier newBeardifier = new Beardifier(((Beardifier) original).pieceIterator, ((Beardifier) original).junctionIterator);
        newBeardifier.setEnhancedPieceIterator(enhancedBeardifierRigidList.iterator());
        newBeardifier.setEnhancedJunctionIterator(enhancedJunctionList.iterator());
     ...
     */
    public static double computeDensity(DensityFunction.FunctionContext ctx, double density, Beardifier data) {
        int x = ctx.blockX();
        int y = ctx.blockY();
        int z = ctx.blockZ();
    ...
    private static void updateAquiferOverrideMask(Beardifier data, AquiferOverride aquiferOverride, int x, int y, int z) {
        NoiseChunk noiseChunk = data.getNoiseChunk();
        NoiseChunk noiseChunkAccessor = (NoiseChunk) noiseChunk;
        AquiferOverrideMaskSupplier aquiferOverrideMaskSupplier = (AquiferOverrideMaskSupplier) noiseChunk;

        int chunkHeight = noiseChunkAccessor.noiseSettings.height();
        int minY = noiseChunkAccessor.noiseSettings.minY();

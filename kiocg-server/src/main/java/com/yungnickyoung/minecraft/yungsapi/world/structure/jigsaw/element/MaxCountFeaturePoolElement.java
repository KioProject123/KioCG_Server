...
public class MaxCountFeaturePoolElement extends FeaturePoolElement implements IMaxCountJigsawPoolElement {
    public static final MapCodec<MaxCountFeaturePoolElement> CODEC = RecordCodecBuilder.mapCodec((builder) -> builder
            .group(
                    PlacedFeature.CODEC.fieldOf("feature").forGetter((featurePoolElement) -> ((FeaturePoolElement)featurePoolElement).feature),
                    projectionCodec(),
    ...
    public String toString() {
        return "MaxCountFeature[" + this.name + "][" + BuiltInRegistries.FEATURE.getKey((((FeaturePoolElement)this).feature.value()).feature().value().feature()) + "][" + this.maxCount + "]";
    }
}

...
public class MaxCountListPoolElement extends ListPoolElement implements IMaxCountJigsawPoolElement {
    public static final MapCodec<MaxCountListPoolElement> CODEC = RecordCodecBuilder.mapCodec((builder) -> builder
        .group(
            StructurePoolElement.CODEC.listOf().fieldOf("elements").forGetter((listPiece) -> ((ListPoolElement)listPiece).getElements()),
            projectionCodec(),
    ...
    public String toString() {
        return "MaxCountList[" + this.name + "][" + ((ListPoolElement)this).getElements().stream().map(Object::toString).collect(Collectors.joining(", ")) + "][" + this.maxCount + "]";
    }
}
